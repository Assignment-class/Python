...
GITHUB_TO_EMAIL_MAP = {
    ...
    "github_user_2": "email_moodle_2@example.com",
...
# --- Langkah Terakhir (BARU): Kirim Nilai ke Moodle (Metode ws.py) ---
# Gantikan blok pengiriman nilai Anda dengan yang ini.

if user_id:
    print(f"Mengirimkan nilai {grade:.2f} (METODE BARU) untuk user {user_id} ke tugas {ASSIGNMENT_ID}...")
    
    # Parameter disesuaikan agar sama persis dengan library ws.py
    grade_params = {
        'wstoken': MOODLE_TOKEN,
        ...
        'userid': user_id,
        'grade': grade, # <-- PERUBAHAN UTAMA: dari 'grade[grade]' menjadi 'grade'
        'attemptnumber': -1, # Mengikuti default dari ws.py (nilai -1 berarti attempt terakhir)
        'addattempt': 0,
        'workflowstate': 'graded',
        'applytoall': 1, # Mengikuti default dari ws.py
    ...
    try:
        ...
        if 'exception' in response.json():
            ...
            # Fungsi ini seringkali mengembalikan array kosong jika sukses
